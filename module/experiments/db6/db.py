...
# select world range
x_range, y_range =      35, 35 # will range from -range to +range
n_bots=                 6       #  number of bots in the world
bot_radius=             0.4    # body-radius of bots - (may depend world on x,y range)
scan_radius=            int((x_range + y_range)*1.5)  # scan-radius of bots - (may depend world on x,y range)
safe_distance=          bot_radius*5    # safety-radius of bots - (may depend world on x,y range and bot_radius)
speed_limit=            bot_radius    # maximum allowed speed in each direction (disctance covered in unit time)
...
def isdF(names, target_points, target_radius, reset_noise, validation=False):
    p,f =  os.path.split(__file__)
    isd = os.path.join(p, isd_dir, )
    return [SwarmState(
        target_point=  (None if target_points is None else target_points[i]) , 
