...
from .common import REMAP, get_angle, get_nspace, RewardSchemes
...
class World(gym.Env):
    ...
    default_reward_scheme = RewardSchemes.scheme_default()
    ...
    def reset(self, starting_state=None, cont=False):
        self.episode += 1
    ...
    def RS_dis_target_point(self): 
        return np.sum ([ np.linalg.norm( self.xy[n, :]-self.TARGET_POINT, 2 )  for n in range(self.N_BOTS) ])

    def RS_dis_target_radius(self): 
        return np.sum ([ np.abs(self.TARGET_RADIUS-np.linalg.norm( self.xy[n, :]-self.TARGET_POINT, 2 ))  for n in range(self.N_BOTS) ])

    _scd_power = 0.5
    def RS_all_unsafe(self):
        #return len(np.where(  (self.dmat<self.SAFE_CENTER_DISTANCE) & (self.dmat>0)  ) [0])
        # self.dmat[np.where((self.dmat<self.SAFE_CENTER_DISTANCE) & (self.dmat>0))]
