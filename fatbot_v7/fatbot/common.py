...
def TEST(
    ...
    for episode in range(episodes):
        ...
        while (not done) and (episode_timesteps<episode_max_steps):
            ...
            renderer.Render() 
            if save_state_info:
                env.save_state(os.path.join(save_state_info, f'{episode}_{env.ts}.npy'))
            
        if save_state_info:
            env.save_state(os.path.join(save_state_info, f'{episode}_{env.ts}_final.npy'))
            fig=env.render() # do a default render
            fig.savefig(os.path.join(save_state_info, f'{episode}_{env.ts}_final.png'))
        #plt.show()
        del fig
        #if cast_render:
        #    _=env.render()
...
def space_filling(start_x=0.0, start_y=0.0, epsilon=1.0, n_iter=3):
    def apply_rules(s):
