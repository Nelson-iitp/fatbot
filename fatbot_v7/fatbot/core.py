...
from .common import REMAP, get_angle, get_nspace, space_filling
...
class World(gym.Env):
    ...
    def add_initial_states(self, *state_list):
        ...
        for state in state_list:
            ...
            for pt in state:
                #print(f'{pt=}, {len(pt)=}')
                point_list = list(space_filling(pt[0], pt[1], epsilon=self.sf_epsilon, n_iter=self.sf_iter))
                #print(f'{point_list=}, {len(point_list)=}')
                ptlist.append(point_list)
            self.initial_states.append(ptlist)
    ...
    def __init__(self, name='default', x_range=10, y_range=10, 
                enable_imaging=True, horizon=0, seed=None, reward_scheme=None, delta_reward=False,
                    ...
                    sf_epsilon = 1.0, # for space-filling
                    sf_iter = 0,
                    #sync_list = [],
        ...
        super().__init__()
        self.name = name
        self.sf_epsilon, self.sf_iter = sf_epsilon, sf_iter
    ...
    def reset(self, starting_state=None):
        ...
        for i,pp in  enumerate(self.initial_states[self.choose_i_state]):
            p = self.rng.choice(a=pp, size=1)
