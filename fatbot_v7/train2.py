...
print(f'{db=}')
#-----------------------------
model_name =            'y'    #<----- stores data in this folder
...
load_model_name, load_model_state = 'x', '0_100.npy'
...
average_return, total_steps = fb.TEST(
    env=            testing_env, 
    model=          fb.PPO.load(os.path.join(load_eval_path, 'best_model')), #<---- use None for random
    episodes=       1,
    steps=          0,
    deterministic=  True,
