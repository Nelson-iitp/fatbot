...
class World_Test(World):
    # world with 2 reward signals

    def set_reward_signal(self):        
        self.reward_labels =          ['d2Target', 'Unsafe']
...
class World_R40(World):
    # world with four reward signals

    def set_reward_signal(self):        
        self.reward_labels =          ['d2Target', 'Unsafe',  'Occluded', 'Neighbour']
        range_multiplier = (1 if self.N_BOTS<2 else self.N_BOTS-1)
        reward_contribution =np.array([ 
                # Norm,                                                    weight
                ((((self.X_RANGE**2+self.Y_RANGE**2)**0.5)*self.N_BOTS),  -1),  # target_dis - lower is better
